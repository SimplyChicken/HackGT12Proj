...
const componentNavItems = [
    ...
    { name: 'Hero', type: 'hero', icon: <Zap className="w-4 h-4" />, href: '/components/hero', isActive: true },
...
// Floating examples component (kept optional)
...
function sanitizeForIframe(src: string) {
    ...
    const sanitized = src
        // strip BOM
        .replace(/^\uFEFF/, '')
        // strip leading markdown fences like ```javascript / ```jsx / ```ts / ```tsx (with optional CRLF)
...
// Simple Hero Preview Component that renders the React code in an iframe
const DynamicHeroPreview = ({ code }: { code: string }) => {
    ...
    useEffect(() => {
        const createPreview = () => {
            try {
                console.log('🔍 Creating hero preview with code:', code);
                const sanitizedCode = sanitizeForIframe(code);
                console.log('🔍 Using sanitized code in iframe:', sanitizedCode);

                // Provide a fallback Hero component if not defined
                const ensureHeroFallback = `
            ...
            function Hero() {
              return React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement('h1', { className: 'text-4xl font-extrabold text-white' }, 'Preview Fallback'),
                  React.createElement('p', { className: 'mt-3 text-blue-100' }, 'Define function Hero() { ... } in your code to override.')
                )
              );
            }
          }
...
<head>
  ...
  <style>
    body { margin:0; padding:0; background:#f8f9fa; font-family: system-ui, -apple-system, sans-serif; width:100vw; height:100vh; overflow-x:hidden; }
...
function HeroCustomizerContent() {
  ...
  return (
    ...
    const getCurrentCode = () => (customizedCode ? customizedCode : defaultHeroCode);
    ...
    const handleCustomize = async () => {
        ...
        setIsLoading(true);
        try {
            const baseCode = customizedCode || defaultHeroCode;

            // Call the Mastra API to customize the hero
            const response = await fetch('/api/mastra', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    action: 'customizeComponent',
                    data: { baseCode, userInput: customizationInput, preferences: {}, componentType: 'hero' },
                }),
            });
            ...
            console.error('Error customizing hero:', error);
        ...
        <div className="min-h-screen bg-eggshell">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="text-center mb-12">
                    <h2 className="text-4xl font-bold text-space-cadet mb-4 font-poly">AI-Powered Hero Customizer</h2>
                        ...
                        {activeTab === 'preview' ? (
                            <div>
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-lg font-semibold text-gray-900">Hero Preview</h3>
                                </div>
                                <div
                                    className="h-[85vh] flex items-center justify-center p-4 overflow-hidden"
                                        ...
                                        <div className="relative w-full max-w-6xl bg-black rounded-2xl p-3 shadow-2xl">
                                            ...
                                            <div className="bg-gray-800 rounded-xl p-1">
                                                ...
                                                <div
                                                    ...
                                                    style={{ aspectRatio: '16/10', height: '500px', overflowY: 'auto', overflowX: 'hidden', width: '100%' }}
                                                >
                                                    <DynamicHeroPreview code={getCurrentCode()} />
                                                </div>
                                            </div>
                                        </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                    <h3 className="text-lg font-semibold text-gray-900 mb-6">Customize Your Hero</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="e.g., add product screenshot, left-aligned content, gradient background, CTA button…"
...
function LoadingFallback() {
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="text-center mb-12">
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">AI-Powered Hero Customizer</h2>
