...
// Simple Navbar Preview Component that renders the React code in an iframe
const DynamicNavbarPreview = ({ code }: { code: string }) => {
  ...
  useEffect(() => {
    const createPreview = () => {
      try {
        console.log('🔍 Creating navbar preview with code:', code);
...
function NavbarCustomizerContent() {
  ...
  // Default navbar template (iframe-safe, desktop only)
  ...
  const handleCustomize = async () => {
    ...
    setIsLoading(true);
    try {
      // Get the current navbar code (starter or previously customized)
      const baseCode = customizedCode || defaultNavbarCode;

      // Call the Mastra API to customize the navbar
      ...
      console.error('Error customizing navbar:', error);
  ...
  const handleFavorite = async () => {
    ...
    try {
      const currentCode = customizedCode || defaultNavbarCode;
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., Make it dark with white text, add a mobile hamburger menu, make it sticky..."
