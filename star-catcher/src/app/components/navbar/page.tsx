...
const componentNavItems = [
    ...
    { name: 'Navbar', type: 'navbar', icon: <Navigation className="w-4 h-4" />, href: '/components/navbar', isActive: true },
    ...
    <div className="relative h-32 overflow-hidden rounded-lg bg-gradient-to-br from-eggshell to-slate-gray/10 p-4">
        <div className="absolute inset-0">
            {examples.map((example, index) => {
                ...
                const positions = ['top-4 left-4', 'top-8 right-8', 'bottom-6 left-6', 'bottom-4 right-4', 'top-12 left-1/2'];
                const animations = ['animate-bounce', 'animate-pulse', 'animate-ping', 'animate-spin', 'animate-pulse'];
                return (
                    <div
                        key={index}
                        ...
                        style={{ animationDelay: `${index * 0.5}s`, animationDuration: `${3 + (index % 3)}s` }}
                    >
...
// Simple custom code highlighter
...
// Sanitizer for iframe execution
function sanitizeForIframe(src: string) {
    const sanitized = src
        ...
        .replace(/^\s*(javascript|jsx|js|typescript|ts|tsx)\s*\r?\n/i, '') // lone language tag
        .replace(/^\s*import\s+.*?;?\s*$/gm, '') // imports
        .replace(/export\s+default\s+function\s+([A-Za-z0-9_]+)\s*\(/g, 'function $1(') // default export fn
        .replace(/export\s+default\s+[A-Za-z0-9_]+\s*;?/g, '') // default export ident
        .trim();
    return sanitized;
}

// Preview iframe
const DynamicNavbarPreview = ({ code }: { code: string }) => {
    ...
    useEffect(() => {
        try {
            ...
            const ensureNavbarFallback = `
        if (typeof Navbar === 'undefined') {
          function Navbar() {
            return React.createElement(
              'nav',
              { className: 'bg-white border-b border-gray-200' },
              React.createElement('div', { className: 'max-w-4xl mx-auto px-4 h-14 flex items-center justify-between' },
                React.createElement('span', { className: 'font-semibold' }, 'Preview Fallback'),
                React.createElement('div', null,
                  React.createElement('a', { href: '#', className: 'text-gray-600 hover:text-gray-900 mx-2' }, 'Home'),
                  React.createElement('a', { href: '#', className: 'text-gray-600 hover:text-gray-900 mx-2' }, 'About')
                )
              )
            );
          }
...
Babel.registerPreset('custom-react', {
  presets: [[Babel.availablePresets['react'], { pragma:'React.createElement', pragmaFrag:'React.Fragment', throwIfNamespace:false }]]
});
</script>
<style>
  body { margin:0; padding:0; background:#f8f9fa; font-family: system-ui, -apple-system, sans-serif; overflow-x:hidden; }
...
<script type="text/babel" data-presets="custom-react">
  ...
  root.render(React.createElement(Navbar));
    ...
    if (error) {
        return <div className="flex justify-center items-center h-32 bg-gray-50 rounded-lg text-red-500">Preview Error: {error}</div>;
    }
...
function NavbarCustomizerContent() {
    ...
    // Default navbar template (iframe-safe)
    const defaultNavbarCode = `function Navbar() {
  return (
    <nav className="bg-white shadow-sm border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center">
            <a href="#" className="text-xl font-bold text-gray-900">Your Brand</a>
          </div>
    ...
    const getCurrentCode = () => (customizedCode ? customizedCode : defaultNavbarCode);

    const handleCustomize = async () => {
        if (!customizationInput.trim()) return;
        setIsLoading(true);
        try {
            const baseCode = getCurrentCode();
            const response = await fetch('/api/mastra', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    action: 'customizeComponent',
                    data: { baseCode, userInput: customizationInput, preferences: {}, componentType: 'navbar' },
                }),
            });
            ...
            console.error(e);
            alert('Failed to customize navbar. Please try again.');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            const response = await fetch('/api/save/components', {
                ...
                body: JSON.stringify({
                    component: {
                        ...
                        name: 'Custom Navbar',
                        code: currentCode,
                        description: `Navbar customized with: ${userInputs.join(', ')}`,
                    },
                    userInputs,
                }),
            });
        ...
        <div className="min-h-screen bg-eggshell">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="text-center mb-12">
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">AI-Powered Navbar Customizer</h2>
                    ...
                    <div className="flex border-b border-gray-200">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-colors ${
                                activeTab === 'preview' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-colors ${
                                activeTab === 'source' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                            }`}
                        >
                        ...
                        {activeTab === 'preview' ? (
                            <div>
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-lg font-semibold text-gray-900">Navbar Preview</h3>
                                </div>
                                <div className="h-[85vh] flex items-center justify-center p-4" style={{ overflowX: 'hidden', overflowY: 'hidden' }}>
                                        ...
                                        <div className="relative w-full max-w-6xl bg-black rounded-2xl p-3 shadow-2xl">
                                            <div className="bg-gray-800 rounded-xl p-1">
                                                <div className="bg-white rounded-lg shadow-inner" style={{ aspectRatio: '16/10', height: '500px', overflowY: 'auto', overflowX: 'hidden', width: '100%' }}>
                                                    <DynamicNavbarPreview code={getCurrentCode()} />
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-gray-900 mb-4 text-center">Source Code</h3>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                    <h3 className="text-lg font-semibold text-gray-900 mb-6">Customize Your Navbar</h3>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="e.g., add dropdowns, mobile menu, brand logo, sticky header…"
                        ...
                        {/* Right column: saved preferences */}
                        <div className="space-y-4">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-16">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-xl font-bold text-gray-900">Navbar Customizer</h1>
                        </div>
                    </div>
                </div>
            </header>
