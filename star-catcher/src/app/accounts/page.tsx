            ...
            {activeTab === "colors" && (
              <>
                ...
                {colorPairs.map((pair, i) => (
                  <PairCard key={`${pair.case_id}-${i}`} colorPair={pair} showColors onDelete={handleDelete} />
                ))}
              </>
            )}
              ...
              <>
                ...
                {combos.length > 0 ? (
                  ...
                  <div className="text-gray-500 text-center py-8">
                    No saved combos yet. Generate some colors and fonts together and click the like button to save them as combos!
                  </div>
                )}
              </>
            )}
                ...
                {savedComponents.length > 0 ? (
                  <div className="space-y-4">
                    {savedComponents.map((component, idx) => (
                      <div key={`${component._id || idx}`} className="bg-white p-6 rounded-lg border border-gray-200">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h4 className="font-semibold text-gray-800 capitalize">{component.type} Component</h4>
                        ...
                        {component.userInputs && component.userInputs.length > 0 && (
                          <div className="mb-4">
                            <h5 className="text-sm font-medium text-gray-700 mb-2">User Inputs:</h5>
                            <div className="space-y-1">
                              {component.userInputs.map((input: string, inputIdx: number) => (
                                <div key={inputIdx} className="text-xs bg-blue-50 text-blue-800 px-2 py-1 rounded">
                                  "{input}"
                                </div>
                              ))}
                            </div>
                        ...
                        {component.analyzedPreferences && (
                          <div className="mb-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                              {component.analyzedPreferences.themes && component.analyzedPreferences.themes.length > 0 && (
                                <div>
                                  <span className="font-medium text-gray-600">Themes:</span>
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {component.analyzedPreferences.themes.map((theme: string, themeIdx: number) => (
                                      <span key={themeIdx} className="px-2 py-1 bg-green-100 text-green-800 rounded">
                                        {theme}
                                      </span>
                                    ))}
                                  </div>
                              ...
                              {component.analyzedPreferences.colors && component.analyzedPreferences.colors.length > 0 && (
                                <div>
                                  <span className="font-medium text-gray-600">Colors:</span>
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {component.analyzedPreferences.colors.map((color: string, colorIdx: number) => (
                                      <span key={colorIdx} className="px-2 py-1 bg-purple-100 text-purple-800 rounded">
                                        {color}
                                      </span>
                                    ))}
                                  </div>
                              ...
                              {component.analyzedPreferences.styles && component.analyzedPreferences.styles.length > 0 && (
                                <div>
                                  <span className="font-medium text-gray-600">Styles:</span>
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {component.analyzedPreferences.styles.map((style: string, styleIdx: number) => (
                                      <span key={styleIdx} className="px-2 py-1 bg-orange-100 text-orange-800 rounded">
                                        {style}
                                      </span>
                                    ))}
                                  </div>
                              ...
                              {component.analyzedPreferences.keywords && component.analyzedPreferences.keywords.length > 0 && (
                                <div>
                                  <span className="font-medium text-gray-600">Keywords:</span>
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {component.analyzedPreferences.keywords.map((keyword: string, keywordIdx: number) => (
                                      <span key={keywordIdx} className="px-2 py-1 bg-gray-100 text-gray-800 rounded">
                                        {keyword}
                                      </span>
                                    ))}
                                  </div>
                  ...
                  <div className="text-gray-500 text-center py-8">
                    ...
                    <p className="text-sm mt-2">Start customizing components to see them here!</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "preferences" && (
              <div className="space-y-6">
                ...
                {userPreferences ? (
                  <div className="space-y-4">
                    {/* AI-Extracted Themes */}
                    {userPreferences.extractedThemes && userPreferences.extractedThemes.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">AI-Extracted Themes</h4>
                        <div className="flex flex-wrap gap-2">
                          {userPreferences.extractedThemes.map((theme, idx) => (
                            <span
                              key={idx}
                    ...
                    {/* AI-Extracted Colors */}
                    {userPreferences.extractedColors && userPreferences.extractedColors.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">AI-Extracted Colors</h4>
                        <div className="flex flex-wrap gap-2">
                          {userPreferences.extractedColors.map((color, idx) => (
                            <span
                              key={idx}
                    ...
                    {/* AI-Extracted Styles */}
                    {userPreferences.extractedStyles && userPreferences.extractedStyles.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">AI-Extracted Styles</h4>
                        <div className="flex flex-wrap gap-2">
                          {userPreferences.extractedStyles.map((style, idx) => (
                            <span
                              key={idx}
                    ...
                    {userPreferences.extractedPatterns && userPreferences.extractedPatterns.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">AI-Extracted Patterns</h4>
                        <div className="flex flex-wrap gap-2">
                          {userPreferences.extractedPatterns.map((pattern, idx) => (
                            <span
                              key={idx}
                    ...
                    {userPreferences.extractedKeywords && userPreferences.extractedKeywords.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">AI-Extracted Keywords</h4>
                        <div className="flex flex-wrap gap-2">
                          {userPreferences.extractedKeywords.map((keyword, idx) => (
                            <span
                              key={idx}
                    ...
                    {userPreferences.styleKeywords && userPreferences.styleKeywords.length > 0 && (
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-semibold text-gray-800 mb-2">Processed Style Keywords</h4>
                        <div className="flex flex-wrap gap-2">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                      <h4 className="font-semibold text-gray-800 mb-2">Analysis Statistics</h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="font-medium text-gray-600">Analysis Count:</span>
                          <span className="ml-2 text-gray-800">{userPreferences.analysisCount || 0}</span>
                        </div>
                        {userPreferences.lastAnalysis && (
                          <div>
                            <span className="font-medium text-gray-600">Last Analysis:</span>
                            <span className="ml-2 text-gray-800">
                              {new Date(userPreferences.lastAnalysis).toLocaleDateString()}
                            </span>
                          </div>
                        )}
                        {userPreferences.lastUpdated && (
                          <div>
                            ...
                            <span className="ml-2 text-gray-800">
                              {new Date(userPreferences.lastUpdated).toLocaleDateString()}
                            </span>
                          </div>
                        )}
                      </div>
